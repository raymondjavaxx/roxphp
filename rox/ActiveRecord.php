...
abstract class Rox_ActiveRecord extends Rox_ActiveModel {
    ...
    public function __construct(array $attributes = null) {
        ...
        if (!isset($this->_behaviors) || empty($this->_behaviors))
            $this->_behaviors = array();

        if (!Rox_Registry::contains(get_class() . 'Behaviors') && !empty($this->_behaviors)) {
            //TODO:: figure out a way to get behaviors for associated models.

            $BehaviorCollection = new Rox_ActiveRecord_BehaviorCollection();
            $this->Behaviors = $BehaviorCollection->setup($this);
            Rox_Registry::addObject(get_class() . 'Behaviors', $this->Behaviors);
        } else {
            $this->Behaviors = Rox_Registry::getObject(get_class() . 'Behaviors');
        }
    }
    ...
    public function __call($method, $args) {
        ...
        //Hooks for Behavior custom functions
        if ($this->Behaviors) {
            $return = $this->Behaviors->dispatchMethod($this, $method, $args);
            if ($return !== FALSE)
                return TRUE;
        }
    ...
    public function save() {
        ...
        if ($this->Behaviors)
            $this->Behaviors->dispatchMethod($this, '_beforeSave');
        $this->_beforeSave();
        ...
        if ($this->_newRecord) {
            ...
            $sql = sprintf(
                    "INSERT INTO `%s` (%s) VALUES (%s)", $this->_table, $attributes, $values
            );
            ...
            if ($dataSource->affectedRows() == 1) {
                ...
                if ($this->Behaviors)
                    $this->Behaviors->dispatchMethod($this, '_afterSave', array(TRUE));
            ...
            $sql = sprintf(
                    "UPDATE `%s` SET %s WHERE `%s` = %s", $this->_table, implode(', ', $updateData), $this->_primaryKey, $this->smartQuote($this->_primaryKey, $this->getId())
            );

            if ($dataSource->execute($sql) !== false) {
                $this->_resetModifiedAttributesFlags();
                if ($this->Behaviors)
                    $this->Behaviors->dispatchMethod($this, '_afterSave', array(FALSE));
    ...
    public function find($ids, $options = array()) {
        ...
        $options = array_merge_recursive(array('attributes' => null, 'order' => null), $options, array('conditions' => array($this->_primaryKey => $ids)));
    ...
    public function delete() {
        ...
        if ($this->Behaviors)
            $this->Behaviors->dispatchMethod($this, '_beforeDelete');
        $this->_beforeDelete();

        $sql = sprintf(
                "DELETE FROM `%s` WHERE `%s` = %s", $this->_table, $this->_primaryKey, $this->smartQuote($this->_primaryKey, $this->getId())
        );
        ...
        if ($deleted) {
            if ($this->Behaviors)
                $this->Behaviors->dispatchMethod($this, '_afterDelete');
            $this->_afterDelete();
        }
    ...
    protected function _validatesUniquenessOf($attributes, $options = array()) {
        ...
        if (!$this->_newRecord) {
            $scopeConditions[] = sprintf("`%s` != %s", $this->_primaryKey, $this->smartQuote($this->_primaryKey, $this->getId()));
        }
